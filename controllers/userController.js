...
exports.profilePage = async (req, res) => {
  try {
    ...
    if (reqUser) {
      ...
      const tweets = await Tweet.find({ author: reqUser._id })
        .populate('author')
        .sort({ created: 'desc' });
...
exports.verifyPage = async (req, res) => {
  res.render('marriage-verification');
};

// Get the registration request page
exports.requestRegPage = async (req, res) => {
  res.render('request-registration');
};
...
exports.verifyRegister = async (req, res, next) => {
  ...
  req
    .checkBody('password-confirm', 'Password confirmation should not be empty')
    .notEmpty();
  req
    .checkBody('password-confirm', 'Both passwords does not match!')
